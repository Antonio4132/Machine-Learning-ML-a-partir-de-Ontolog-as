...
def convert_graph(data):
    ...
    if len(sentence) <3:
        ...
        for r in new_result[0]: 
            if not ("<http://www.semanticweb.org/alicia/ontologies/2020/8/singleCellRepositories#" in r or "SubClassOf" in r or "instanceOf" in r):
                print(r)
        return new_result
...
def generate_graph(axiom_file, instances_file):
    ...
    with open(axiom_file, "r") as f: # Abre el archivo de axiomas
        for line in f.readlines():
            result = convert_graph(line.strip()) # Para cada linea, llama a convert_graph
    ...
    print("COMIENZA EL DE INSTANCIAS")
    with open(instances_file, "r") as f: # Abre el archivo de instancias
        for line in f.readlines():
            result = convert_graph(line.strip()) # Para cada linea, llama a convert_graph
